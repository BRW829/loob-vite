...
const DEFAULT_BRANDS = ['Tealive', 'Bask Bear Coffee', 'Ayee Southern Thai', 'Gindaco', 'Croissant Taiyaki', 'Ding Dang']
...
const FORM_SECTIONS = [
  {
    title: 'Identity', cols: 3, fields: [
      ...
      { k: 'brand', l: 'Brand', opts: '__BRANDS__' },
...
const DB_FIELDS = [
  ...
  { key: 'monthlySales', label: 'Avg Monthly Sales' }, { key: 'atv', label: 'ATV' }, { key: 'gto', label: 'GTO %' },
...
function toDB(o) {
  return {
    ...
    service_charge: o.serviceCharge || null, ap_fees: o.apFees || null,
...
function fromDB(r) {
  return {
    ...
    serviceCharge: r.service_charge || '', apFees: r.ap_fees || '',
    ...
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter,"Segoe UI",sans-serif' }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: '48px 44px', width: 400, boxShadow: '0 8px 40px rgba(0,0,0,0.10)', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 14 }}>
          <input type={show ? 'text' : 'password'} value={pw} onChange={e => { setPw(e.target.value); setErr('') }} onKeyDown={e => { e.stopPropagation(); if (e.key === 'Enter') attempt() }} placeholder="Enter password" autoFocus style={{ width: '100%', padding: '12px 44px 12px 16px', borderRadius: 10, border: `1.5px solid ${err ? '#FCA5A5' : C.border}`, fontSize: 15, outline: 'none', boxSizing: 'border-box', color: C.text }} />
          <button onClick={() => setShow(!show)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: C.sub, fontSize: 15 }}>{show ? '🙈' : '👁️'}</button>
        </div>
        {err && <div style={{ background: C.redBg, color: C.red, borderRadius: 8, padding: '10px 14px', fontSize: 13, marginBottom: 14 }}>{err}</div>}
        <button onClick={attempt} style={{ width: '100%', background: C.accent, color: '#fff', border: 'none', borderRadius: 10, padding: '13px', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>Sign In →</button>
...
// ── Brand Manager ─────────────────────────────────────────────────────────────
function BrandManager({ allBrands, customBrands, onAdd, onDelete }) {
  const [open, setOpen] = useState(false)
  const [newBrand, setNewBrand] = useState('')
  const [err, setErr] = useState('')
  function handleAdd() {
    if (!newBrand.trim()) return
    const ok = onAdd(newBrand)
    if (ok) { setNewBrand(''); setErr('') }
    else setErr('Brand already exists')
  }
  return (
    <div style={{ marginBottom: 10 }}>
      <button onClick={() => setOpen(!open)} style={{ width: '100%', background: '#fff', border: `1px solid ${C.border}`, color: C.sub, padding: '9px', borderRadius: 9, cursor: 'pointer', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span>🏷 Manage Brands</span><span>{open ? '▲' : '▼'}</span>
      </button>
      {open && (
        <div style={{ background: '#F8FAFC', border: `1px solid ${C.border}`, borderRadius: 9, padding: 12, marginTop: 6 }}>
          <div style={{ fontSize: 12, color: C.sub, fontWeight: 700, marginBottom: 6 }}>DEFAULT</div>
          {DEFAULT_BRANDS.map(b => <div key={b} style={{ fontSize: 12, color: C.sub, padding: '3px 0', borderBottom: `1px solid ${C.border}` }}>{b}</div>)}
          {customBrands.length > 0 && <>
            <div style={{ fontSize: 12, color: C.sub, fontWeight: 700, margin: '10px 0 6px' }}>CUSTOM</div>
            {customBrands.map(b => (
              <div key={b} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: `1px solid ${C.border}` }}>
                <span style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{b}</span>
                <button onClick={() => onDelete(b)} style={{ background: C.redBg, border: 'none', color: C.red, padding: '2px 8px', borderRadius: 5, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>✕</button>
              </div>
            ))}
          </>}
          <div style={{ marginTop: 10 }}>
            <input value={newBrand} onChange={e => { setNewBrand(e.target.value); setErr('') }} onKeyDown={e => { e.stopPropagation(); if (e.key === 'Enter') handleAdd() }} placeholder="New brand name..." style={{ width: '100%', padding: '7px 10px', borderRadius: 7, border: `1px solid ${err ? C.red : C.border}`, fontSize: 13, outline: 'none', boxSizing: 'border-box', marginBottom: 6 }} />
            {err && <div style={{ fontSize: 11, color: C.red, marginBottom: 6 }}>{err}</div>}
            <button onClick={handleAdd} style={{ width: '100%', background: C.accent, border: 'none', color: '#fff', padding: '7px', borderRadius: 7, cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>+ Add Brand</button>
          </div>
        </div>
      )}
    </div>
  ...
  async function doImport() {
    ...
    for (let i = 0; i < all.length; i += BATCH) {
      const batch = all.slice(i, i + BATCH).map(o => ({
        outlet_code: o.outletCode, outlet_format: o.outletFormat, brand: o.brand, outlet_name: o.outletName,
        state: o.state, type: o.type, model: o.model, status: o.status || 'Active',
        opening_date: o.openingDate || null, closure_date: o.closureDate || null,
        sqft: toNum(o.sqft), storage_size_sqft: toNum(o.storageSizeSqft), lat: o.lat, lng: o.lng,
        outlet_address: o.outletAddress, storage_address: o.storageAddress,
        monthly_rental: toNum(o.monthlyRental), monthly_gross_rent: toNum(o.monthlyGrossRent),
        monthly_sales: toNum(o.monthlySales), atv: toNum(o.atv), gto: toNum(o.gto),
        service_charge: toNum(o.serviceCharge), ap_fees: toNum(o.apFees),
      ...
      try {
        ...
        done.push(...(await res.json()).map(fromDB)); setPct(Math.min(100, Math.round(((i + BATCH) / all.length) * 100)))
        ...
        <div>
          <div style={{ ...card, marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: busy ? 16 : 0 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={doImport} disabled={busy} style={{ background: busy ? '#93C5FD' : C.accent, border: 'none', color: '#fff', padding: '9px 22px', borderRadius: 9, cursor: busy ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 700, minWidth: 190 }}>{busy ? `Importing... ${pct}%` : `Import ${csv.rows.length} Outlets →`}</button>
              </div>
            </div>
          ...
          <div style={{ ...card, overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead><tr style={{ borderBottom: `2px solid ${C.border}` }}>{['Code', 'Brand', 'Outlet Name', 'State', 'Status', 'Base Rent', 'Avg Sales', 'Lease Expiry', 'Franchisee'].map(h => <th key={h} style={{ padding: '10px 12px', textAlign: 'left', color: C.sub, fontWeight: 700, fontSize: 13 }}>{h}</th>)}</tr></thead>
...
// ── Render field (outside OutletForm to prevent focus loss) ───────────────────
function renderField(f, form, set, brands) {
  ...
  const val = form[f.k] ?? ''
  const resolvedOpts = f.opts === '__BRANDS__' ? brands : f.opts
  const el = resolvedOpts
    ? <select value={val} onChange={e => set(f.k, e.target.value)} onKeyDown={stopKey} style={inp}><option value=''>— Select —</option>{resolvedOpts.map(o => <option key={o}>{o}</option>)}</select>
    : f.textarea
...
// ── Outlet Form ───────────────────────────────────────────────────────────────
function OutletForm({ initial, onSave, onCancel, saving, brands }) {
  ...
  return (
    <div>
      {FORM_SECTIONS.map(sec => (
        <div key={sec.title} style={{ marginBottom: 22 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${sec.cols},1fr)`, gap: 12 }}>
            {sec.fields.map(f => renderField(f, form, set, brands))}
          </div>
        </div>
      ))}
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 8, paddingTop: 16, borderTop: `1px solid ${C.border}` }}>
        ...
        <button onClick={() => onSave(form)} disabled={saving} style={{ background: saving ? '#93C5FD' : C.accent, border: 'none', color: '#fff', padding: '10px 26px', borderRadius: 9, cursor: saving ? 'not-allowed' : 'pointer', fontSize: 15, fontWeight: 700 }}>{saving ? 'Saving...' : 'Save Outlet'}</button>
      </div>
    </div>
  )
}
...
function AppContent({ role, onLogout }) {
  ...
  async function save(data) {
    ...
    try {
      ...
      toast$(data.id ? 'Outlet updated' : 'Outlet created'); setEditing(null)
  ...
  if (!ready) return <div style={{ background: C.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter,sans-serif' }}><div style={{ textAlign: 'center', color: C.sub }}><div style={{ fontSize: 40, marginBottom: 10 }}>⟳</div><div style={{ fontSize: 16 }}>Loading...</div></div></div>
      ...
      <div style={{ width: 230, background: C.white, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', padding: '24px 0', flexShrink: 0, boxShadow: '2px 0 8px rgba(0,0,0,0.04)' }}>
        <div style={{ padding: '0 20px 28px' }}>
          ...
          <div style={{ marginTop: 6 }}>
            <span style={{ background: isAdmin ? C.accentBg : C.greenBg, color: isAdmin ? C.accent : C.green, fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 99 }}>{isAdmin ? 'Admin' : 'Viewer'}</span>
          </div>
        </div>
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 20 }}>
                {[
                  ...
                  { l: 'Portfolio Sales', v: 'RM ' + (stats.sales / 1000).toFixed(0) + 'K/mo', sub: 'avg monthly, active outlets', c: C.green },
                  { l: 'Avg R/S Ratio', v: stats.avgRS + '%', sub: parseFloat(stats.avgRS) > 15 ? 'Above target — review rents' : 'Within target', c: parseFloat(stats.avgRS) > 15 ? C.red : C.green },
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
                {[
                  { l: 'Total Monthly Gross Rent', v: fRM(stats.rental) },
                    ...
                    return (
                      <div key={b} style={{ marginBottom: 16 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                          <span style={{ fontSize: 14, fontWeight: 700, color: BRAND_COLOR[b] || C.accent }}>{b}</span>
          ...
          {view === 'outlets' && (
            <div>
              <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <select value={filterBrand} onChange={e => setFilterBrand(e.target.value)} onKeyDown={stopKey} style={{ background: '#fff', border: `1px solid ${C.border}`, color: C.text, padding: '9px 13px', borderRadius: 9, fontSize: 14, outline: 'none' }}>{allBrandsWithAll.map(b => <option key={b}>{b}</option>)}</select>
                      ...
                      {filtered.map((o, i) => {
                        ...
                        return [
                          <tr key={o.id} onClick={() => setExpanded(isExp ? null : o.id)} style={{ background: isExp ? C.accentBg : i % 2 === 0 ? '#fff' : '#FAFBFD', cursor: 'pointer' }}>
                            ...
                            <td style={TD}>{o.monthlyGrossRent ? 'RM ' + Number(o.monthlyGrossRent).toLocaleString() : '—'}</td>
                          ...
                          isExp && (
                            <tr key={o.id + '-x'}>
                              <td colSpan={isAdmin ? 17 : 16} style={{ padding: '18px 20px', background: '#F0F6FF', borderBottom: `1px solid ${C.border}` }}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 14, marginBottom: 14 }}>
                                  {[
                                    ...
                                    ['Service Charge', fRM(o.serviceCharge) + '/mo'],
                                    ['A&P Fees', fRM(o.apFees) + '/mo'],
                                    ['Security Dep', fRM(o.securityDeposit)],
                                    ['Utilities Dep', fRM(o.utilitiesDeposit)],
                                    ['Fitout Dep', fRM(o.fitoutDeposit)],
                                    ['Restoration Dep', fRM(o.restorationDeposit)],
                                    ['Mailbox Dep', fRM(o.mailboxDeposit)],
                                    ['Other Deposit', fRM(o.otherDeposit)],
                                    ['Other Dep Remark', o.otherDepositRemark || '—'],
                                    ['Total Deposits', fRM(dep(o)), true],
          ...
          {view === 'lease' && (
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 12, marginBottom: 20 }}>
                {[['Security', 'securityDeposit'], ['Utilities', 'utilitiesDeposit'], ['Fitout', 'fitoutDeposit'], ['Restoration', 'restorationDeposit'], ['Mailbox', 'mailboxDeposit'], ['Other', 'otherDeposit']].map(([l, k]) => {
                  const t = outlets.reduce((s, o) => s + (Number(o[k]) || 0), 0)
                  return <div key={l} style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}><div style={{ fontSize: 11, color: C.sub, fontWeight: 700, marginBottom: 4 }}>{l.toUpperCase()}</div><div style={{ fontSize: 16, fontWeight: 800, color: C.amber }}>RM {(t / 1000).toFixed(0)}K</div></div>
                })}
              </div>
              <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr>{['Code', 'Brand', 'Outlet', 'Landlord', 'Tenant', 'Lease Expiry', 'Status', 'LAD', 'Sec', 'Util', 'Fitout', 'Rest', 'MBX', 'Other', 'Other Remark', 'TOTAL'].map(h => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                    <tbody>
                      {[...outlets].sort((a, b) => (days(a.leaseExpiry) ?? 9999) - (days(b.leaseExpiry) ?? 9999)).map((o, i) => (
                        <tr key={o.id} style={{ background: i % 2 === 0 ? '#fff' : '#FAFBFD' }}>
                          ...
                          <td style={TD}>{fRM(o.otherDeposit)}</td>
                          <td style={{ ...TD, color: C.sub, fontSize: 12 }}>{o.otherDepositRemark || '—'}</td>
                    ...
                    <tfoot>
                      <tr style={{ background: '#F8FAFC', borderTop: `2px solid ${C.border}` }}>
                        ...
                        {['securityDeposit', 'utilitiesDeposit', 'fitoutDeposit', 'restorationDeposit', 'mailboxDeposit', 'otherDeposit'].map(k => (
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 20 }}>
                {allBrands.map(b => {
                  ...
                  const r = bos.reduce((s, o) => s + (Number(o.monthlyGrossRent) || Number(o.monthlyRental) || 0), 0)
                  ...
                  return (
                    <div key={b} style={{ background: '#fff', border: `1px solid ${C.border}`, borderLeft: `4px solid ${BRAND_COLOR[b] || C.accent}`, borderRadius: 14, padding: '16px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                      <div style={{ color: BRAND_COLOR[b] || C.accent, fontWeight: 800, fontSize: 15, marginBottom: 12 }}>{b}</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        {[['Active', bos.length, null], ['Avg Sales/mo', 'RM ' + (s / 1000).toFixed(0) + 'K', C.green], ['Gross Rent/mo', 'RM ' + (r / 1000).toFixed(0) + 'K', null], ['R/S', rs ? rs + '%' : '—', rs && parseFloat(rs) > 15 ? C.red : rs ? C.green : C.sub]].map(([l, v, c]) => (
              ...
              <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                <div style={{ padding: '14px 18px', borderBottom: `1px solid ${C.border}`, fontSize: 14, fontWeight: 700, color: C.sub, letterSpacing: 1 }}>OUTLET RANKING — Active, by Avg Monthly Sales</div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr>{['#', 'Code', 'Brand', 'Outlet', 'State', 'Sqft', 'Avg Sales/mo', 'Base Rent', 'Gross Rent', 'SC', 'A&P', 'R/S', 'Sales/sqft', 'ATV', 'GTO Amt'].map(h => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                    <tbody>
                      {outlets.filter(o => o.status === 'Active' && o.monthlySales).sort((a, b) => Number(b.monthlySales) - Number(a.monthlySales)).map((o, i) => {
                        ...
                        const gtoAmt = o.gto && o.monthlySales ? Math.round(Number(o.monthlySales) * Number(o.gto) / 100) : null
                        return (
                          <tr key={o.id} style={{ background: i % 2 === 0 ? '#fff' : '#FAFBFD' }}>
                            ...
                            <td style={TD}>{fRM(o.monthlyRental)}</td>
                            <td style={TD}>{fRM(o.monthlyGrossRent)}</td>
                            ...
                            <td style={{ ...TD, color: C.sub }}>{gtoAmt ? 'RM ' + gtoAmt.toLocaleString() + ' (' + o.gto + '%)' : '—'}</td>
                          </tr>
                        )
                      })}
                    </tbody>
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }} onKeyDown={e => e.stopPropagation()}>
          <div style={{ background: '#fff', borderRadius: 16, width: 820, maxHeight: '92vh', overflowY: 'auto', padding: 28, boxShadow: '0 8px 40px rgba(0,0,0,0.15)' }}>
            ...
            <OutletForm initial={editing} onSave={save} onCancel={() => setEditing(null)} saving={saving} brands={allBrands} />
          </div>
        </div>
      )}
    </div>
